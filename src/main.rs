...
//            .wrap(auth)
            ...
            .service(web::scope("/questions")
                .route("/", web::get().to(api::get_questions))
                .route("/{id}", web::get().to(api::get_question_by_id))
                .route("/", web::post().to(api::add_question))
                .route("/{id}", web::delete().to(api::delete_question))
            )
            .service(web::scope("/response")
                .route("/", web::get().to(api::get_responses))
                .route("/{id}", web::get().to(api::get_response_by_id))
                .route("/", web::post().to(api::add_response))
                .route("/{id}", web::delete().to(api::delete_response))
            )
            }
        )
        .bind("127.0.0.1:8989")?
